...
(() => {
    ...
    function selectorFor(element) {
        ...
        while (
            ...
            const classes = [...node.classList]
                .filter((name) => !name.startsWith("fw-"))
                .slice(0, 2);
            if (classes.length > 0) {
    ...
    async function captureScreen() {
        if (!navigator.mediaDevices?.getDisplayMedia) {
            throw new Error(
                "Screen-Capture wird von diesem Browser nicht unterstützt.",
            );
        }
    ...
    function initWidget(root) {
        ...
        const openPanel = () => {
            panel.hidden = false;
            root.querySelector("[data-fw-open]").setAttribute(
                "aria-expanded",
                "true",
            );
        };
        ...
        root.querySelector("[data-fw-close]").addEventListener(
            "click",
            closePanel,
        );
        $("[data-fw-pick]").addEventListener("click", () =>
        ...
        $("[data-fw-shot]").addEventListener("click", async () => {
            ...
            try {
                ...
                setStatus(
                    `${error.message} Alternativ Bilddatei wählen.`,
                    true,
                );
            }
        });
        ...
        submitButton.addEventListener("click", async () => {
            ...
            if (message.length < 3) {
                setStatus(
                    "Bitte kurz beschreiben, worum es geht (min. 3 Zeichen).",
                    true,
                );
                return;
            }
